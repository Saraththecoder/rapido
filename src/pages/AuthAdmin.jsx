...
export default function AuthAdmin() {
  ...
  const quickLogin = () => {
    setLoading(true);
    setTimeout(() => {
      login('admin@swiftgo.com', 'SwiftGo@2026');
      setLoading(false);
      navigate('/');
    }, 400);
  };
      ...
      <div className="flex-1 flex flex-col justify-center px-6 py-8 z-10">
        <div className="w-full max-w-[320px] mx-auto space-y-6">
          ...
          <div className="flex items-center gap-2 p-2.5 rounded-xl bg-rose-50 border border-rose-200">
            ...
            <span className="text-[10px] font-bold text-rose-600">Enhanced security required for this portal</span>
          </div>

          {/* Error */}
          {error && (
            <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }}
              className="p-3 rounded-xl bg-rose-50 text-rose-600 text-xs font-bold border border-rose-200 text-center">
              {error}
            </motion.div>
          )}
          ...
          >
            <div className="space-y-3">
              <div className="space-y-1">
                ...
                <div className="relative">
                  <input
                    type="text"
                    placeholder="admin@swiftgo.com"
                    value={email}
                    onChange={(e) => setEmail(e.target.value)}
                    className="w-full bg-white border border-gray-200 rounded-xl pl-9 pr-4 py-3 text-xs font-bold text-gray-800 focus:outline-none focus:border-rose-400 focus:ring-2 focus:ring-rose-100 transition-all shadow-sm"
                  />
                  <Mail className="w-4 h-4 text-rose-400 absolute left-3 top-3.5" />
                </div>
              </div>
            ...
            <motion.button
              ...
              type="submit"
              disabled={loading}
              className="w-full py-3.5 bg-gradient-to-r from-rose-500 to-slate-700 text-white font-black text-xs uppercase tracking-widest rounded-xl shadow-lg shadow-rose-300/20 flex items-center justify-center gap-2 hover:from-rose-600 hover:to-slate-800 transition-all disabled:opacity-70"
            >
          ...
          <div className="border-t border-gray-100 pt-4">
            ...
            <button
              onClick={quickLogin}
              className="w-full flex items-center justify-between px-3 py-2.5 rounded-xl bg-rose-50 border border-rose-200 hover:bg-rose-100 transition-all"
            >
              <div className="flex items-center gap-2">
                <Shield className="w-3.5 h-3.5 text-rose-500" />
                <span className="text-xs font-bold text-gray-700">Admin Console (SwiftGo)</span>
              </div>
